<template>
  <div>
    ...
    <EnterpriseModal @register="registerModal" @success="handleSuccess" />
  </div>
</template>
<script lang="ts">
...
import { searchEnterpriseApi, deleteEnterpriseApi } from '/@/api/enterprise';
import { NewsSearchItem } from '/@/api/enterprise/newsModel';

import { useModal } from '/@/components/Modal';
import { useMessage } from '/@/hooks/web/useMessage';
import EnterpriseModal from './EnterpriseModal.vue';

import { columns, getFormConfig, searchFormSchema } from './enterprise.data';

export default defineComponent({
  name: 'enterprise-management',
  components: { BasicTable, EnterpriseModal, TableAction, Icon, TableImg },
  setup() {
    ...
    const [registerTable, { reload }] = useTable({
      ...
      api: (params) => {
        return new Promise(async (resolve) => {
          let res = await searchEnterpriseApi(Object.assign(searchData.value, {
    ...
    async function handleDelete(record: Recordable) {
      await deleteEnterpriseApi({ id: record.id });
